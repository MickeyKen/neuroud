...
class Env2():
    def __init__(self, is_training):
        ...
        self.pub_cmd_vel = rospy.Publisher('/ubiquitous_display2/cmd_vel', Twist, queue_size=10)
        self.sub_odom = rospy.Subscriber('/gazebo/model_states', ModelStates, self.getPose)
        self.sub_jsp = rospy.Subscriber('/ubiquitous_display2/joint_states', JointState, self.getJsp)
        self.reset_proxy = rospy.ServiceProxy('gazebo/reset_world', Empty)
        self.unpause_proxy = rospy.ServiceProxy('gazebo/unpause_physics', Empty)
        self.pause_proxy = rospy.ServiceProxy('gazebo/pause_physics', Empty)
        self.goal = rospy.ServiceProxy('/gazebo/spawn_sdf_model', SpawnModel)
        self.del_model = rospy.ServiceProxy('/gazebo/delete_model', DeleteModel)
        self.pan_pub = rospy.Publisher('/ubiquitous_display2/pan_controller/command', Float64, queue_size=10)
        self.tilt_pub = rospy.Publisher('/ubiquitous_display2/tilt_controller/command', Float64, queue_size=10)
        self.image_pub = rospy.Publisher('/ubiquitous_display2/image', Int32, queue_size=10)
    ...
    def setUDposition(self, x, y):
        ...
        try:
            ...
            srv.model_state.model_name = 'ubiquitous_display2'
    ...
    def getPose(self, pose):
        self.position = pose.pose[pose.name.index("ubiquitous_display2")]
        self.ud_x = pose.pose[pose.name.index("ubiquitous_display2")].position.x
        self.v = pose.twist[pose.name.index("ubiquitous_display2")].linear.x
        ...
        if arrive and reach:
            ...
            while not human:
                ...
                try:
                    ...
                    target = SpawnModel
                    target.model_name = 'actor1'  # the same with sdf name
                    ...
                    self.goal(target.model_name, target.model_xml, 'namespace2', self.goal_position, 'world')
                except (rospy.ServiceException) as e:
                    print("/gazebo/failed to build the target")
                rospy.sleep(0.1)
                ...
                while data is None:
                    try:
                        data = rospy.wait_for_message('/ubiquitous_display2/scan_filtered', LaserScan, timeout=5)
                    except:
                        pass
    ...
    def find_human(self, data):
        ...
        for count in range(3):
            for i, item in enumerate(data.ranges):
                ...
                if x < 8.4 and x > 1.6 and y < 5.4 and y > 2.6:
                    human_count += 1
            if human_count > 7:
                tf_count += 1
        if tf_count >= 2:
