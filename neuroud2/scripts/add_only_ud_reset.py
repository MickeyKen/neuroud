...
def step(action, last_action):
    ...
    if action == 0: #STOP
        ...
        if last_action == 0 or last_action == 3 or last_action == 33:
...
def reset():
    ...
    try:
        #reset_proxy.call()
        reset_model_msg = SetModelStateRequest()
        reset_model_msg.model_state.model_name = 'ubiquitous_display'
        reset_model(reset_model_msg)
    except (rospy.ServiceException) as e:
