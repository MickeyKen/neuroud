...
def step(action, last_action):
    ...
    if action == 0: #STOP
        ...
        pan_msg = Float64()
        pan_msg.data = random.uniform(-2.9670, 2.9670)
        pan_pub.publish(pan_msg)
        print pan_msg.data

    elif action == 4:
        tilt_msg = Float64()
        tilt_msg.data = random.uniform(-0.2617, 1.3)
        tilt_pub.publish(tilt_msg)
        print tilt_msg.data

    elif action == 5:
        img_msg = Int32()
        img_msg.data = random.randrange(2)
...
if __name__ == '__main__':
    ...
    tilt_pub = rospy.Publisher('/ubiquitous_display/tilt_controller/command', Float64, queue_size=10)

    image_pub = rospy.Publisher('/ubiquitous_display/image', Int32, queue_size=10)
