...
class ActorNetwork:
	...
	def create_network(self,state_dim, action_dim):
		...
		action_linear = self.batch_norm_layer(action[:, None, 0],training_phase=is_training,scope_bn='action_linear',activation=tf.tanh)
		action_pan = self.batch_norm_layer(action[:, None, 1], training_phase=is_training, scope_bn='action_pan', activation=tf.tanh)
		action_tilt = self.batch_norm_layer(action[:, None, 2], training_phase=is_training, scope_bn='action_tilt', activation=tf.tanh)
		# action_linear = tf.sigmoid(action[:, None, 0])
		# action_angular = tf.tanh(action[:, None, 1])
		action = tf.concat([action_linear, action_pan, action_tilt], axis=-1)
	...
	def create_target_network(self,state_dim,action_dim,net):
		...
		action_linear = self.batch_norm_layer(action[:, None, 0], training_phase=is_training, scope_bn='target_action_linear', activation=tf.tanh)
		action_pan = self.batch_norm_layer(action[:, None, 1], training_phase=is_training, scope_bn='target_action_pan', activation=tf.tanh)
		action_tilt = self.batch_norm_layer(action[:, None, 2], training_phase=is_training, scope_bn='target_action_tilt', activation=tf.tanh)
		# action_linear = tf.sigmoid(action[:, None, 0])
		# action_angular = tf.tanh(action[:, None, 1])
		action = tf.concat([action_linear, action_pan, action_tilt], axis=-1)
